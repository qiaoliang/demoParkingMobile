...
public class BackendService extends Service implements Runnable {
...
//            return;
        ...
        if (socket == null)// 如果未连接到服务器，创建连接
            connectService();
        if (!BackendService.this.td.isAlive())// 如果当前线程没有处于存活状态，重启线程
            (td = new Thread(BackendService.this)).start();
...
//                writer.println(msg);
    ...
    public IBinder onBind(Intent intent) {
        ...
        td = new Thread(BackendService.this);// 启动线程
        td.start();
